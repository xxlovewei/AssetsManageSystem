function userRoleAdjustFormCtl($localStorage, notify, $log, $uibModal,
	...
	$scope.userRoles = []
	$http
			.post($rootScope.project + "/api/role/roleQueryFormatKV.do", {})
			.success(
					function(res) {
						if (res.success) {
	...
	$scope.sure = function() {
		...
		$http.post($rootScope.project + "/api/user/userRoleChange.do", ps)
				.success(function(res) {
					if (res.success) {
...
function sysUserSettingCtl(DTLang, DTOptionsBuilder, DTColumnBuilder, $compile,
	...
	function flush() {
		...
		if ($scope.userGroupSel.group_id != "ALL") {
			$scope.URL = $rootScope.project
					+ "/api/user/userQueryByGroup.do?group_id="
					+ $scope.userGroupSel.group_id;
		} else {
			$scope.URL = $rootScope.project + "/api/user/userQueryByGroup.do";
		}
