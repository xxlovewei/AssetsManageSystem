function sysTaskCtl(DTLang, DTOptionsBuilder, DTColumnBuilder, $compile,
	...
	function renderAction(data, type, full) {
		...
		acthtml = acthtml + " <button ng-click=\"enable('" + full.seq
				+ "')\" class=\"btn-white btn btn-xs\">启用</button>  ";
		acthtml = acthtml + " <button ng-click=\"disable('" + full.seq
				+ "')\" class=\"btn-white btn btn-xs\">停用</button>  ";

		acthtml = acthtml + " <button ng-click=\"pause('" + full.seq
				+ "')\" class=\"btn-white btn btn-xs\">暂停</button>  ";
		acthtml = acthtml + " <button ng-click=\"resume('" + full.seq
				+ "')\" class=\"btn-white btn btn-xs\">恢复</button>  ";

		acthtml = acthtml + " <button ng-click=\"runonce('" + full.seq
				+ "')\" class=\"btn-white btn btn-xs\">执行一次</button> </div> ";
		return acthtml;
	}
	...
	$scope.dtColumns = [
			...
			DTColumnBuilder.newColumn('jobname').withTitle('任务名称').withOption(
					'sDefaultContent', ''),
			DTColumnBuilder.newColumn('jobcontent').withTitle('内容').withOption(
					'sDefaultContent', ''),
			DTColumnBuilder.newColumn('jobenable').withTitle('初始化').withOption(
					'sDefaultContent', '').renderWith(renderStatus),
			DTColumnBuilder.newColumn('jobrunstatus').withTitle('运行状态')
					.withOption('sDefaultContent', '').renderWith(renderRunStatus),
	...
	function flush() {
		...
		$http.post($rootScope.project + "/api/schedule/queryJobs.do", ps)
