...
function setAttr(fileName) {
	...
	$.ajax({
		  ...
		  data:{'cmd':'attr', 'fileFileName':fileName, 'permissions':$('#permissions').val()},
...
//	$.post($path+'/api/sftp/exeCommand.do', {'cmd':'attr', 'fileFileName':fileName, 'permissions':$('#permissions').val()}, function(data){
...
function exec(cmd, cmdParam) {
	...
	$.ajax({
		  type: 'POST',
		  url: $path+'/api/sftp/exeCommand.do',
		  data: {'cmd':cmd, 'cmdParam':cmdParam},
		  success: function(data){
			  showFiles(data);
		  },
		  dataType:"json",
...
//	
//	$.post($path+'/api/sftp/exeCommand.do', {'cmd':cmd, 'cmdParam':cmdParam}, function(data){
//		showFiles(jQuery.parseJSON(data));
//	});
};
...
//	
	...
	$(data.data.files).each(function(i, e){
		if (e.directory) {
