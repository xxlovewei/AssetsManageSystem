...
function zcHcoutCtl(DTOptionsBuilder, DTColumnBuilder, $compile, $confirm,
    ...
    $scope.dtColumns.push(DTColumnBuilder.newColumn('inwarehousestr').withTitle('入库仓库')
        .withOption('sDefaultContent', ''));
    $scope.dtColumns.push(DTColumnBuilder.newColumn('busidate').withTitle($rootScope.USEDCOMP_IN)
        .withOption('sDefaultContent', ''));
    $scope.dtColumns.push(DTColumnBuilder.newColumn('inusedcompname').withTitle($rootScope.USEDPART_IN)
        .withOption('sDefaultContent', ''));
