...
public class ResTranferService extends BaseService {
    ...
    public R startFlow(String pinst, String busid, String ifsp) {
        if ("1".equals(ifsp)) {
            ...
            ups.eq("busid", busid);
            ResTranferServiceImpl.update(ups);
            sure(busid, SysProcessDataService.PSTATUS_DTL_FINISH_NO_APPROVAL);
        }
    ...
    public R sure(String busid, String status) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            e.setType(ZcCommonService.ZC_BUS_TYPE_ZY);
            e.setFillct("1");
            e.setCdate(new Date());
    ...
    public R save(ResTranfer entity, String items) {
        ...
        if (ToolUtil.isNotEmpty(id)) {
            ...
            busid = zcService.createUuid(ZcCommonService.UUID_ZY);
            //当前方案设置结束流程
        ...
        //锁定单据中的数据
        String sql2 = "update res_tranfer_item a,res b set b.inprocess='1',b.inprocessuuid='" + busid + "',b.inprocesstype='" + ZcCommonService.ZC_BUS_TYPE_ZY + "' where a.resid=b.id and a.busid=? and b.dr='0' and a.dr='0'";
        db.execute(sql2, busid);
