...
public class ResTranferExtController extends BaseController {
    ...
    public R approval(String busid) {
        ...
        if(rs2.size()>0){
            ...
            ins.set("dr","0");
            ins.set("busid",busid);
            ins.set("provider", BaseFlowService.APPOINTDEPARTMENT_APPROVAL_ROLE);
        ...
        ins2.set("provider", BaseFlowService.FIX_APPROVAL_ROLE);
        db.execute(ins2);
        ...
        res.put("ptype", ZcCommonService.ZC_BUS_TYPE_ZY);
        res.put("psubtype",ZcCommonService.ZC_BUS_TYPE_MYZY);
        String code=ZcCommonService.ZC_BUS_TYPE_ZY;
        String tpl="";
