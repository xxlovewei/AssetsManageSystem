 ...
 */
@Controller
@RequestMapping("/api/cmdb/resLabelTpl")
public class ResLabelTplController extends BaseController {
    ...
    public R deleteById(@RequestParam(value = "id", required = true, defaultValue = "") String id) {
        return R.SUCCESS_OPER(ResLabelTplServiceImpl.removeById(id));
    }
    ...
    public R selectById(@RequestParam(value = "id", required = true, defaultValue = "") String id) {
        return R.SUCCESS_OPER(ResLabelTplServiceImpl.getById(id));
    }

    @ResponseBody
    @Acl(info = "插入", value = Acl.ACL_USER)
    @RequestMapping(value = "/insert.do")
    public R insert(ResLabelTpl entity) {
        return R.SUCCESS_OPER(ResLabelTplServiceImpl.save(entity));
    }

    @ResponseBody
    @Acl(info = "根据Id更新", value = Acl.ACL_USER)
    @RequestMapping(value = "/updateById.do")
    public R updateById(ResLabelTpl entity) {
        return R.SUCCESS_OPER(ResLabelTplServiceImpl.updateById(entity));
    }

    @ResponseBody
    @Acl(info = "存在则更新,否则插入", value = Acl.ACL_USER)
    @RequestMapping(value = "/insertOrUpdate.do")
    public R insertOrUpdate(ResLabelTpl entity) {
        return R.SUCCESS_OPER(ResLabelTplServiceImpl.saveOrUpdate(entity));
    }
    ...
    public R selectList() {
        return R.SUCCESS_OPER(ResLabelTplServiceImpl.list(null));
    }
    ...
    public R selectPage(String start, String length, @RequestParam(value = "pageSize", required = true, defaultValue = "10") String pageSize, @RequestParam(value = "pageIndex", required = true, defaultValue = "1") String pageIndex) {
        ...
        QueryWrapper<ResLabelTpl> ew = new QueryWrapper<ResLabelTpl>();
        //ew.and(i -> i.eq("user_id", getUserId()).apply(pagesize>10, "rtime>sysdate-1","23"));
        IPage<ResLabelTpl> pdata = ResLabelTplServiceImpl.page(new Page<ResLabelTpl>(pageindex, pagesize), ew);
