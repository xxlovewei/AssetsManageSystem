...
import com.dt.core.common.dao.sql.Block;
import com.dt.core.common.dao.sql.Delete;
import com.dt.core.common.dao.sql.Insert;
import com.dt.core.common.dao.sql.SE;
...
public abstract class SpringDAO   
{
    ...
    public int[] batchExecute(String sql,List... ps){
		...
		int listSize=-2;
		if(ps.length>0) listSize=ps[0].size();
	...
	//不同数据库的分页实现
	protected abstract RcdSet getPageSet(RcdSet set,String sql,int pageSize,
			int pageIndex,Map<String, Object> params);
 
	
	/**
	 * 如果pageSize不为0,则不分页
	 * */
	public RcdSet queryPage(String sql, int pageSize, 
			int pageIndex,Map<String, Object> params)
	{
		if (params == null)
		...
		if(totalRecord==0) totalRecord=set.size();
			...
			Object... params)
	{
		...
		{
			querySql=translate(querySql);
			this.jdbcTemplate.query(querySql, new RcdRowMapper(set),
					params);
		}
		set.setPageInfos(pageSize, pageIndex, totalRecord, totalPage,sql);
		return set;
...
//	}
	...
	public static void main(String[] args) {
		//System.out.println(translateToCountSQL("select a,b,c from tab,ggg where a=b and c=d group by dd having c>0 order by d desc nulls last"));
	}
