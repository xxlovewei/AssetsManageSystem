...
public class OrderBy extends SubSQL
{
	...
	public String getSQL() {
		...
		for(int i=0;i<ses.size();i++)
		{
			...
			sql.append(join(ses.get(i).getSQL(),orders.get(i).toString(),(nulls.get(i)==null?"":SQLKeyword.NULLS.toString()+SQLKeyword.SPACER.toString()+nulls.get(i).toString())));
	...
	public String getParamedSQL() {
		...
		{
			ses.get(i).setIgnorColon(ignorColon);
			sql.append(join(ses.get(i).getParamedSQL(),orders.get(i).toString(),(nulls.get(i)==null?"":SQLKeyword.NULLS.toString()+SQLKeyword.SPACER.toString()+nulls.get(i).toString())));
			if(i<ses.size()-1) sql.append(SQLKeyword.COMMA);
		}
		return sql.toString();
	}
	...
	public String getParamNamedSQL() {
		...
		{
			ses.get(i).setIgnorColon(ignorColon);
			sql.append(join(ses.get(i).getParamNamedSQL(),orders.get(i).toString(),(nulls.get(i)==null?"":SQLKeyword.NULLS.toString()+SQLKeyword.SPACER.toString()+nulls.get(i).toString())));
			if(i<ses.size()-1) sql.append(SQLKeyword.COMMA);
		}
